...
function ChatPage({ user, setUser }) {
  ...
  useEffect(() => {
    ...
    // Join the user's room (backend expects username as string)
  ...
  useEffect(() => {
    const subscribe = async () => {
      try {
        ...
        const { key } = await fetch(`${API_URL}/vapidPublicKey`).then(r => r.json())
        if (!key) return
        ...
        await fetch(`${API_URL}/subscribe`, {
          ...
          body: JSON.stringify({ username: user, subscription: sub })
        })
      } catch (e) {
        // swallow
      }
  ...
  // Setup socket connection once per user
  useEffect(() => {
    // Listen for incoming messages (backend emits "message")
    socket.current.on("message", (newMessage) => {
      // Only add message if it's for the currently selected user
      console.log(messages);
      if (
        (newMessage.sender === selectedUser && newMessage.receiver === user) ||
        (newMessage.sender === user && newMessage.receiver === selectedUser)
      ) {
        setMessages(prev => [...prev, newMessage])
      }
    })

    // Debug: log connection/disconnection
  ...
  const handleSend = async (message) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { sender: user, receiver: selectedUser, message }])
    } catch (err) {
